...
function createTaskCard(task) {
  ...
  const assignedToHTML = task.assignedTo
    ? `<div style="font-size: 12px; color: #94a3b8;">Assigned: ${task.assignedTo}</div>`
    : "";

  const dueDateHTML = task.dueDate
    ? `<div style="font-size: 12px; color: #94a3b8;">Due: ${new Date(task.dueDate).toLocaleDateString()}</div>`
    : "";
